...
#: Review 4986931575 BLOCKING 1: SS-L condition 7's own record, on its own identity.
CLOSURE_ID = "5900000003"
RUN_ID = "3100000001"
JOB_ID = "9500000001"
...
class FakeGit:
    ...
    def __init__(self, **overrides):
        self.parents = {
            ...
            # BOUND, so its exact parent order and merge-tree identity are actually verified. The
            # placeholder pair here previously passed only because those checks were disabled; the
            # stand-in now serves the same real identities the object store holds.
            AUTH.CORRECTION_AUTHORIZING_MERGE_SHA: (
                AUTH.CORRECTION_AUTHORIZING_MERGE_BASE,
                AUTH.CORRECTION_AUTHORIZING_ACCEPTED_HEAD,
            ),
            ...
            # EXTENDED BY XASSET-0049. The step-8-equivalent rebinding verifies TWO further merges
            # from git -- the PRIOR ANCHOR it supersedes (XASSET-0047 / PR #347, a CLOSED
            # predecessor, not a stopped one) and its own AUTHORITY (XASSET-0048 / PR #348) -- so
            # an honest stand-in vouches for each separately rather than letting an unknown anchor
            # pass. These are the REAL identities, so the fixture agrees with the object store.
            AUTH.PRIOR_RECONCILIATION_MERGE_SHA: (
                AUTH.PRIOR_RECONCILIATION_MERGE_BASE,
                AUTH.PRIOR_RECONCILIATION_ACCEPTED_HEAD,
            ),
        ...
        self.trees = {
            ...
            # XASSET-0049: each added merge's tree equals its accepted head's tree, which is
            # exactly the zero-merge-drift property the module PROVES rather than assumes. TWO
            # DISTINCT synthetic trees, so a swap between the two new entries is still caught.
            AUTH.PRIOR_RECONCILIATION_MERGE_SHA: "9" * 40,
...
class FakeGovernance:
    ...
    def __init__(self, **overrides):
        ...
        self.comments = {
            ...
            # BLOCKING 1 (review 4986931575): SS-L condition 7. Authored by the lifecycle
            # operator, naming the exact merge and CI run, and created strictly AFTER both the
            # post-merge verification (13:00) and the CI job's completion (13:30).
            CLOSURE_ID: {
        ...
        self.runs = {
            RUN_ID: {
                ...
                "conclusion": "success",
                "head_sha": MERGE,
                "updated_at": "2026-08-16T13:30:00Z",
            }
        }
        self.jobs = {
            JOB_ID: {
                ...
                "conclusion": "success",
                "head_sha": MERGE,
                "completed_at": "2026-08-16T13:30:00Z",
            }
        }
        self.__dict__.update(overrides)
...
def lifecycle() -> dict:
    return {
        ...
        # BLOCKING 1 (review 4986931575): XASSET-0044 SS-L condition 7, now a required, closed,
        # actor-bound, identity-bound and chronology-bound gate rather than an unchecked claim.
        "lifecycle_closure": {
...
class TestExactBaseAndMergeDrift:
    ...
    def test_d5_reviewed_base_is_bound_to_the_current_lifecycles_own_base(self):
        ...
        #
        # RE-ANCHORED AGAIN BY XASSET-0049, unchanged in KIND for the fourth time, and
        # STRENGTHENED. The current lifecycle is the XASSET-0048-authorized step-8-EQUIVALENT
        # rebinding, which branches from the XASSET-0048 merge, so THAT is what it must bind.
        # XASSET-0047's own accepted value is retained immediately below as history rather than
        ...
        # XASSET-0047's own accepted base, retained verbatim as history: it WAS the XASSET-0046
