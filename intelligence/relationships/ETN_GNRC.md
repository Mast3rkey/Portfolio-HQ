...
directly grepped `intelligence/companies/ETN.yaml` and
`intelligence/companies/ETN.md` for "GNRC" and "Generac" and found zero
matches — Eaton's own Company Intelligence record does not independently
name Generac, a shared customer, or a shared project anywhere. No attempt
was made to locate independent ETN-side primary-source confirmation of a
named joint relationship — that would constitute new external research,
which this unit is not authorized to perform.

Neither `intelligence/companies/GNRC.yaml`/`GNRC.md` nor
`intelligence/companies/ETN.yaml`/`ETN.md` was read for the purpose of
...
same data-center power stack" alongside Eaton (ETN)'s switchgear and
power-distribution equipment, GE Vernova (GEV)'s turbines and grid
equipment, and Quanta Services (PWR)'s EPC/interconnection services — four
functionally distinct, non-substitutable equipment/service categories
...
`symmetric: true`, with GNRC and ETN as co-equal subjects, per REL-0001 §D's
rule that a symmetric type is recorded once per pair with no directional
subject/object.

## Why this pair, why this batch

Per the inventory audit's §9 candidate table, `GNRC_ETN` (alongside
`GNRC_GEV` and `GNRC_PWR`) was identified as an evidence-ready second-batch
...
complementary to ETN's (and GEV's and PWR's) role in the same data-center
electrical buildout.

**Not established by this record:** any named joint contract, shared
hyperscaler customer, or shared project specifically linking GNRC and ETN
(no source located names both companies together in a single disclosed
transaction); any dollar figure or percentage attributable to the GNRC-ETN
overlap specifically, as distinct from GNRC's total data-center backlog;
whether GNRC's data-center pivot will continue to grow relative to its
majority-residential revenue base (GNRC.md §13 explicitly frames this as
"a blended, actively evolving exposure," not settled); any measured
historical price correlation between GNRC and ETN or any `power_infra`
member — GNRC.md §13 states explicitly that no such correlation scan has
ever been performed by this record or by any prior repository decision;
...
- **`duplicate_exposure_detection`** — GNRC's C&I/data-center leg and ETN's
...
or ETN's tier, target, role, cluster, cap, or holding — GNRC's cluster
