...
directly grepped `intelligence/companies/GEV.yaml` and
`intelligence/companies/GEV.md` for "GNRC" and "Generac" and found zero
matches — GE Vernova's own Company Intelligence record does not
independently name Generac, a shared customer, or a shared project
anywhere. No attempt was made to locate independent GEV-side primary-source
...
`intelligence/companies/GEV.yaml`/`GEV.md` was read for the purpose of
...
same data-center power stack" alongside GE Vernova (GEV)'s turbines and
grid equipment, Eaton (ETN)'s switchgear and power-distribution equipment,
and Quanta Services (PWR)'s EPC/interconnection services — four
...
`symmetric: true`, with GEV and GNRC as co-equal subjects, per REL-0001
§D's rule that a symmetric type is recorded once per pair with no
directional subject/object.

## Why this pair, why this batch

Per the inventory audit's §9 candidate table, `GNRC_GEV` (alongside
`GNRC_ETN` and `GNRC_PWR`) was identified as an evidence-ready second-batch
...
complementary to GEV's (and ETN's and PWR's) role in the same data-center
electrical buildout.

**Not established by this record:** any named joint contract, shared
hyperscaler customer, or shared project specifically linking GNRC and GEV
(no source located names both companies together in a single disclosed
transaction); any dollar figure or percentage attributable to the GNRC-GEV
...
historical price correlation between GNRC and GEV or any `power_infra`
...
- **`duplicate_exposure_detection`** — GNRC's C&I/data-center leg and GEV's
...
or GEV's tier, target, role, cluster, cap, or holding — GNRC's cluster
