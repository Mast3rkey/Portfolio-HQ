# AMZN ↔ MSFT — competitor (symmetric)
...
§7 ("Genuine diversification versus duplicated exposure") and, for the
separately-noted regulatory context only, §6 ("Antitrust and regulatory
exposure — correlated or company-specific?") — an already existing,
already-merged comparison artifact, not either company's own individual
record. The advisory batch recommendation in
...
businesses measured three ways." Separately, §6 notes AMZN and MSFT are
both named in the same specific EU DMA cloud-gatekeeper preliminary
proceeding — disclosed here as related context, not folded into this
record's own `competitor` claim.
...
`symmetric: true`, with AMZN and MSFT as co-equal subjects.
...
assessment that AMZN and MSFT compete directly as public-cloud sellers for
overlapping enterprise AI-infrastructure spending, while retaining a
genuinely different competitive approach each; and that both companies are
separately named together in the same EU DMA regulatory proceeding.

**Not established by this record:** any dollar figure, market-share
percentage, or win/loss data quantifying the AMZN-MSFT competitive overlap
specifically; independent, company-specific risk-disclosure language from
either AMZN or MSFT's own record restating this competitive finding in
their own words; whether holding both AMZN and MSFT alongside
...
record does not attempt to resolve it; any measured historical price
correlation between AMZN and MSFT (REL-0001 §G explicitly separates
...
- **`duplicate_exposure_detection`** — AMZN and MSFT are both governed
...
- **`stress_testing`** — a shared demand-side AI-capex-cycle risk, and a
  shared specific regulatory proceeding, across two directly competing
  cloud sellers are disclosed, structural facts a future portfolio-level
  stress scenario could reference.
...
or MSFT's tier, target, role, cluster, cap, or holding; any trade or
