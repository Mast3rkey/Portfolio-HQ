...
**Methodology:** the evidentiary research underlying this document was read-only repository inspection (file reads, code tracing, `git log`/`git show`, GitHub PR metadata for historical context only) — during that research, no code was fixed, no parameter was changed, no calibration research was run, and no live Alpaca/brokerage/account-state fetch was performed. This document was subsequently filed as a new documentation artifact through the separately authorized `NUM-0001` governance pull request, which necessarily created a branch, this file, and a commit to add it to the repository — that ordinary filing act is distinct from, and does not retroactively change, the read-only character of the underlying research.

## Label vocabulary

The following labels are used where shown to distinguish verified facts, repository claims, inferences, uncertainties, recommendations, and principal decisions. Absence of a label on a table cell does not elevate that cell's authority — every row in the matrix below is held to the same evidentiary standard whether or not a label is printed next to it.
...
Historical PR bodies and prior chat/session text (including PR #117's and #119's descriptions) are referenced in this document only as historical record of what was discussed or merged — never as repository authority. Operational authority follows the complete hierarchy `governance/decisions/GOV-0002-operational-precedence-hierarchy.md` establishes, highest first: (1) the Investment Constitution, including only explicitly incorporated material within its incorporated scope; (2) accepted governance decisions, including `decision_log.yaml` only as authoritative historical evidence within its original scope; (3) current accepted or frozen specifications and doctrine not incorporated at level 1; (4) `CLAUDE.md`; (5) `targets.yaml`; (6) `holdings.yaml`; (7) executable code and tests, as implementation and verification, never as policy originators; (8) generated outputs and non-authoritative syntheses. This audit document sits at level 8 — non-authoritative synthesis/evidence — unless a higher-authority source explicitly gives it a narrower incorporated role; `NUM-0001` does not do so.
...
## 4. External-Term and Observed-State Freshness Register

This register covers two distinct classes, each labeled explicitly per row: parameters classified **externally imposed** (a broker/regulatory term the system did not choose) and facts that are **observed state** (a synced account fact — not itself one of the six provenance classes in §2, but included here because it shares the same freshness-disclosure need: source, date, and defined staleness behavior). Margin debt and the margin buffer percentage are observed state, not externally imposed parameters — they are account facts pasted in from a live brokerage screen, not a term the broker imposes on the system's design. No single universal freshness interval is applied across this register — each cadence or stale-threshold is itself consequential and is recorded with its own rationale below.

| Parameter | Class | Source | Observed/verified date | Account applicability | Point-in-time or durable | Refresh owner | Refresh condition | Behavior when stale/unavailable |
|---|---|---|---|---|---|---|---|---|
| Margin interest rate (~5% APR, first $1,000 free) | Externally imposed | Robinhood margin account terms | **UNCERTAIN — no verification date recorded anywhere in this repository** | This specific Robinhood account | Point-in-time (broker-set, can change) but currently documented as if durable | Account holder (no API exposes this rate; Robinhood's own maintenance-ratio internals aren't available via Alpaca per `holdings.yaml`'s own comment) | None stated — **this is itself a gap** | Undefined — no fallback, no warning if the rate has since changed |
| Robinhood-displayed margin buffer % | Observed state | Robinhood UI, pasted manually | `holdings.yaml`'s `synced_at` field — **VERIFIED** present and populated (2026-07-20 as of this audit's preceding session) | This specific account | Point-in-time by design (a live account state, not a durable term) | Account holder, via the documented `update-margin` workflow | Event-driven — "before any large margin-funded buy" per `CLAUDE.md`; also checked every run against `STALE_MARGIN_DAYS` | `STALE_MARGIN_DAYS` (2 days) triggers a warning banner in `render()` — a working example of the pattern the margin-rate row above lacks |
| Margin debt figure | Observed state | Robinhood UI, pasted manually | Same `synced_at` field as buffer % | Same | Point-in-time | Account holder | Same event-driven condition | Same staleness warning |
...
- **`crypto.sleeve_pct` has two independent hardcoded fallbacks for the identical config key**: `allocate.py`'s `plan()` (`sleeve_pct = float(crypto_cfg.get("sleeve_pct", 0))`, audited-SHA line reference ~375) defaults to `0` for the sleeve gap computation; `allocate.py`'s `main()` (`result["crypto"] = {..., "sleeve_pct": float(crypto_cfg.get("sleeve_pct", 10)), ...}`, audited-SHA line reference ~1143) defaults to `10` when constructing the result dictionary that `render()` later reads for display. The fallback lives in `main()`, not in `render()` itself — `render()` only consumes the already-built dictionary and has no fallback of its own. Canonical owner: `targets.yaml:42` (`crypto.sleeve_pct: 10.0`). **Masked** — the key is explicitly set today, so both paths agree. Consequence if exercised: a missing/misconfigured key would make the rendered advisory ("vs. 10% target") contradict the actual computed allocation gap (against 0%).
...
- **`leverage_cap`/`buffer_floor_pct` fall back to hardcoded `1.8`/`30.0`** in `allocate.py`'s `plan()` — a *missing* `margin:` block defaults through `targets.get("margin", {})` to an empty mapping, and a missing `leverage_cap` or `buffer_floor_pct` key within that mapping then defaults individually via `.get(key, default)` — with no warning distinguishing "explicitly configured" from "key absent, silently defaulted." Canonical owner: `targets.yaml:123-124`. **Masked** — both values are explicitly present and match the fallback today. This audit did not establish that a *malformed* (present-but-wrong-type, e.g. a non-mapping) `margin:` value follows the same silent-fallback path — calling `.get()` on a non-mapping value raises an exception rather than silently substituting a default, so a malformed value more plausibly fails loudly than silently; this distinction is not resolved by inference here. Consequence if exercised (missing block/key only): the two most safety-critical parameters in the system would silently continue operating at the same numbers with no error.
...
|---|---|---|
| P1-1 | Silent `leverage_cap`/`buffer_floor_pct` fallbacks | Would require the `margin:` block or one of its keys to be *absent* from `targets.yaml` — not demonstrated. A *malformed* (wrong-type) value was not established by this audit to follow the same silent-fallback path; it more plausibly raises an exception than silently defaults. |
