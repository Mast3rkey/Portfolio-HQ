...
**Update (2026-07-17, adopted for future research only):** the criteria above ("Evidence supports" = material gap AND repeated forced-deleveraging events) are exactly what governed Phase 4A's actual execution and its recorded "Evidence inconclusive" outcome — unchanged, preserved as the historical record. Following `docs/PHASE4A_OUTCOME_GATE_REVIEW.md`'s governance review, the second condition is revised for **future** research cycles only: material gap AND (repeated forced-deleveraging events **OR** material threshold-exposure degradation, per `docs/PHASE4A_MATERIALITY_THRESHOLDS.md` §6's ≥5%-of-days bar). Rationale: the event-count condition alone was too narrow — a coverage gap, not a miscalibrated number — since it detects only one specific passive-breach sequence, while Phase 4A's own already-collected time-above-threshold data showed a far larger, more consistent signal (100.0% of days above the reference threshold once concentrated, vs. 61.7%/14.0% at baseline) that the original gate never consulted. Full rationale in `docs/PHASE4A_OUTCOME_GATE_REVIEW.md` §4 and `docs/PHASE4A_MATERIALITY_THRESHOLDS.md` §8b. **This is a future research-methodology change, not a retroactive reclassification** — Phase 4A's own recorded outcome, report, and JSON are not edited or recomputed.
