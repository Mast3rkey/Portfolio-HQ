...
**Last verified against repository sources at commit `38394ddab127583c61549a3f5361df0b3cfd472c`.** Later changes to authoritative sources supersede this synthesis. This marker records when the manual's prose was last checked against its sources — it does not make the manual itself authoritative; see the sourcing statement above.
...
- **30% margin-buffer floor, hard cutoff.** Before any margin-funded buy, the *projected* post-trade buffer is checked; if it would drop below 30%, that buy is blocked outright — **no partial taper.** A synced buffer already below 30% is itself a forced de-lever signal (trim or pay down margin), same severity as a cash-tight guardrail.
