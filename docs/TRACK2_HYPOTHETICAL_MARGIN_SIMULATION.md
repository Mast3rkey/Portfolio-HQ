# Track 2 — Hypothetical Margin Policy Simulation

_2026-07-17 · Documentation only. No code changes. This is the research specification for Track 2 of the Phase 1 margin research program (see `docs/MARGIN_DOCTRINE.md` for the governing philosophy, `docs/MARGIN_INTELLIGENCE_DESIGN.md` for the full engine design this gates, `docs/MARGIN_DATA_INVENTORY.md` for the A/B/C data classification this track's inputs are drawn from). Not implemented until reviewed and approved._

> ⚠️ **Every result this track will ever produce is hypothetical by construction, and must be labeled as such wherever it's cited.** This track does not answer "what did this account's margin actually do" — that question is Track 1's (`docs/TRACK1_HISTORICAL_REALITY_AUDIT.md`), and Track 1's own conclusion is that the real data to answer it doesn't exist. This track answers a different, related question instead: **given the portfolio construction rules this system actually uses, what would a stated, explicit margin policy have done to a simulated account run forward through real historical prices?** That's a legitimate, useful question — it's just not the same question, and conflating the two would misrepresent a simulation as a historical fact. Every number in this document (once run) gets a "(hypothetical, simulated)" tag; none of it should ever be cited as "what this account's margin has done."

**Question this track answers:** given the portfolio construction rules, what would different leverage policies have done historically to a simulated account? (Not: what did this account's real margin actually do — see Track 1.)
...
**Consequence:** Portfolio A cannot be "this account's real history." `docs/TRACK1_HISTORICAL_REALITY_AUDIT.md` now documents exactly what real data *does* exist (7 discrete margin sync snapshots, one recorded withdrawal, zero interest-paid records, nothing before 2026-07-13) — Track 1 is the authoritative answer to "what do we actually know," and this track does not duplicate or re-derive it. Every other backtest in this repo (`backtest_regime.py`, `backtest_trend.py`, `backtest_trims.py`, `backtest_weights.py`, `backtest_rungs.py`, `backtest_t1t2_trim.py`) already resolves an identical problem the same way — none of them replay this account's real deposit/trade history either. They all simulate a **hypothetical portfolio**, running the current production rules (universe, weight scheme, deposit cadence, trim rules) forward through the real historical price window (`data/backtest/*.json`, 2021–2026). This track uses that same, already-established method — placing it squarely in `MARGIN_DATA_INVENTORY.md`'s **Category B (reconstructable estimates)**, never Category A.

**What this means concretely:** Portfolio A ("hypothetical, with margin") and Portfolio B ("hypothetical, without margin") are both simulated portfolios, identical in every respect (universe, weights, deposits, trim rules) except for one variable: whether a stated, explicit **margin-draw policy** is active. Section 2 defines that policy precisely, because — unlike the price data — there is no real historical policy to inherit either. The Decisions Log's 1.8x cap was set at "the level already in place when discovered," not derived from a stated draw policy, so Section 2 has to propose one rather than replay one. **Neither portfolio in this track is, or claims to be, this account.**
...
- **Net negative** → Phase 2 does not proceed automatically. Per the standing framing ("only after we know margin is useful"), a net-negative Track 2 result is itself the answer to whether margin has been useful — running a leverage-level sweep in that case would be tuning a mechanism the evidence just argued against, not answering an open question. This becomes a decision point for you, not an automatic next step: continue investigating anyway (e.g., a different repayment discipline might rescue a currently-negative result — that's a legitimate reason to still proceed), or pause the Margin Intelligence build entirely pending that discussion.

---

## 8. Exact artifacts Track 2 must produce before Phase 2 (leverage sweep) begins
...
6. **Every number in the report and the Decisions Log entry carries a "(hypothetical, simulated)" tag** — matching this document's own opening banner — so nothing produced here can later be quoted as a fact about this account's real margin usage without the qualifier attached. Track 1 (`docs/TRACK1_HISTORICAL_REALITY_AUDIT.md`) remains the only source for real, observed facts about this account's margin.

None of this is built yet. This document is the specification only.

---

## Note on not assuming an outcome

Restated for the record since it's the operating principle of this entire document: nothing above proposes, hints at, or is structured to favor any particular answer to "is margin useful." The classification thresholds in Section 7 were chosen for symmetry (same ±0.10 band regardless of direction, same 5pp drawdown tolerance regardless of which arm is worse) specifically so the conclusion is determined by the backtest's output, not by this specification's framing. And regardless of what Track 2 eventually concludes, it will remain a conclusion about a **simulated** account — Track 1's finding that this account's real margin history is largely unrecoverable does not change, and does not get papered over by a well-designed simulation.
