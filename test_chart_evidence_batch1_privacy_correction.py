...
def test_workstreams_active_branch_cleared_after_pr227_merged(workstreams_text):
    """PR #227 (branch claude/chart-0002-batch1-postmerge-correction) has
    completed its full independent-review/principal-acceptance/merge/
    post-merge-verification lifecycle and its source branch is deleted --
    active_branch/active_pr must no longer point to it as if still open,
    matching this repository's own established convention (e.g. the
    WS-0006/WS-0009 entries) of nulling these fields once nothing is
    pending or blocked for the referenced PR."""
    ws0012_start = workstreams_text.index("- id: WS-0012")
    next_id = workstreams_text.find("\n  - id: WS-", ws0012_start + 1)
    ws0012_text = workstreams_text[ws0012_start: next_id if next_id != -1 else None]
    assert "active_branch: null" in ws0012_text
    assert "active_pr: null" in ws0012_text
    assert "active_branch: claude/chart-0002-batch1-postmerge-correction" not in ws0012_text
