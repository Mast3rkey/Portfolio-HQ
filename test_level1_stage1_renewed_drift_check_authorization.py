...
class TestIdentitiesAreDerivedFromTheBoundMerge:
    def test_the_boundary_is_exactly_eighteen_unique_paths(self):
        ...
        order, unique, and none traded away -- proved against the AST-derived historical set."""
        _assert_boundary_is_the_bound_set_plus_authorized_additions()

    @pytest.mark.parametrize("relpath", sorted(HISTORICAL_LOAD_BEARING))
    def test_every_bound_path_is_byte_identical_to_the_bound_merge(self, relpath):
        # RE-ANCHORED AGAIN BY XASSET-0060: parametrised over the HISTORICAL set this unit
        # actually bound, derived by AST from the immutable bound merge. The seven paths
        # XASSET-0060 lawfully ADDED did not exist in this merge at all, so asking whether they
        # are "byte-identical to" it was unanswerable rather than merely weaker; their identity is
        # bound by XASSET-0060's own suite. Coverage of the eighteen is unchanged.
...
class TestFilingTimeBytesAreUnchanged:
    ...
    def test_the_module_constants_are_untouched(self):
        ...
        THIS unit bound are retained as NEGATIVE PINS -- still named, still asserted, now as the
        values the anchor moved OFF -- and the successor's own values are pinned positively, so an
        unauthorized move to any third value fails both halves.
        """
