...
2. Positions are **live-priced automatically** (2026-07-13): `holdings.yaml`'s `shares:` block (ticker → share count) is the source of truth for every normal position — value = qty × latest Alpaca price, recomputed on every run, no manual resync needed for price moves. After a real buy/sell/trim changes a share count, paste `TICKER qty` lines → run `update-shares` (**merge mode; never `--replace` without my explicit confirmation**). SKHY (permanent zero Alpaca coverage) and the crypto sleeve (BTC/ETH/SOL — no share-count data) stay on the old manual dollar-value path: paste screenshots or `TICKER value` lines → run `update-holdings`. Paste the margin screen too when it's changed → run `update-margin <debt> <buffer_pct>` (use Robinhood's own **displayed** buffer %, never derive it — see `allocate.py`'s `write_state()` comment for why).
