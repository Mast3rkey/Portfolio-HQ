...
- Governed Company Intelligence — business model, segments, economics, financial facts, disclosed risks, catalysts, and sources (`sector`, `industry`, `themes`, `competitive_advantages`, `risks[]`, `sources[]`, and the paired `.md` thesis narrative **after** the redaction pass specified in §7 — the `.md` file is evidence, not exempt from redaction; see §7's corrected scope);
- Theme Intelligence (`intelligence/themes/`) where directly relevant to a ticker's economic role;
- governed `intelligence/relationships/*.yaml`/`.md` records, where permitted by sequencing (§9 — never before `economic_role`/`capital_priority` are sealed; this is a sequencing rule this filing establishes for Milestone 6 specifically, consistent with, and extending, `TIER-0002` §3.5's description of `risk_concentration` as a pure cross-reference rollup — `TIER-0002` itself states no ordering requirement of its own);
...
**Corrected scope, this version.** The paired `.md` thesis narrative is **not** exempt from this problem. An earlier version of this artifact claimed the `.md` file was "evidence-only by construction... and passes through unredacted" — that claim is **false** and has been removed. Direct inspection of the current 27-ticker population found `portfolio_role_ref` values, tier labels, conviction restatements, or prior committee/policy-conclusion language in plain prose inside at least 8 of 27 `.md` files (`TSM.md:404` — "`portfolio_role_ref: T1` was explicitly approved by the human principal..."; `AVGO.md:253` — "`portfolio_role_ref: T2` reflects `targets.yaml`'s current tier placement..."; `NVDA.md:352-353` — "The PI-0017 committee review's own advisory policy recommendation was 'Keep current policy'..."; `GEV.md` — committee-review/`conviction.rating` references; `AMZN.md`, `ISRG.md`, `TMO.md`, `CEG.md` — current tier placement (T1/T2) and conviction-rationale discussion in prose). Company Intelligence's frozen spec does not itself embed tier/target/gate conclusions in the `.md` template, but nothing prevents — and in practice several records' own thesis narratives do contain — later-added prose that restates or discusses `portfolio_role_ref`, tier placement, or a prior committee/policy conclusion, typically while explaining *why* a rating or placement is what it is. The `.md` file therefore requires the same redaction discipline as the `.yaml` file, not none.

### 7.2 Mechanical redaction, specified (not implemented in this filing)

A single deterministic redaction procedure, reused identically across all 27 tickers, applied to **both** `intelligence/companies/<TICKER>.yaml` **and** `intelligence/companies/<TICKER>.md`:

**Applied to the `.yaml` file:**
...
3. **Retain unchanged**: `sector`, `industry`, `themes`, `competitive_advantages`, `risks[]` (all sub-fields, including `severity`), `sources[]`. These are the permitted evidence.

**Applied to the `.md` file (corrected — previously specified as a pass-through, which this version retracts):**

4. **Strip any paragraph or sentence** containing one or more of the following markers, case-insensitive: `portfolio_role_ref`, a bare tier label used as a policy placement (`T1`, `T2`, `band`, `spec`, `ETF`, `gated` — used as *this ticker's own current placement*, not as generic vocabulary), `conviction.rating`, `conviction stays`/`conviction is` when discussing the record's own rating level, `keep current policy`, `committee review`, `promoted to`/`promoted from`/`retained at`, and `approved by the human principal` in a placement/rating context. This is a **sentence- or paragraph-level strip**, not a whole-document exclusion — the surrounding business-model, risk, catalyst, and thesis-mechanism prose in the same file is retained.
5. **Mandatory post-redaction verification, not a one-pass trust**: after step 4, re-scan the redacted `.md` output against the same marker list. **Any remaining match is a hard failure** — the redaction procedure must not silently ship a package containing an unstripped marker; a future implementation must either re-run the strip pass, fall back to excluding that specific paragraph entirely, or (if the marker cannot be cleanly isolated from surrounding legitimate evidence) exclude the ticker's `.md` narrative for that drafting pass and note the exclusion in that ticker's `evidence_quality` axis rather than ship contaminated evidence. This two-pass discipline (strip, then verify the strip actually worked) exists because free-form prose is not a fixed key list like the `.yaml` file's structure — a single pattern-match pass demonstrated by this artifact's own corrected §7.1 finding is not something to trust blind.

**Applied to both files:**

6. **Scan and strip**: any reference, anywhere in the retained content, to a chart image filename, a `governance/evidence/CHART-0001/` or `governance/evidence/CHART-0002/` path, or a technical/price-action term — a defensive pass; live inspection of all 27 Company Intelligence records found zero such references today (chart evidence lives exclusively under `governance/evidence/CHART-*/`, never inside `intelligence/companies/`), but the mechanical scan step is specified so a future record change cannot silently reintroduce a leak.
7. **Output**: one sanitized package per ticker (redacted `.yaml` content plus redacted `.md` content), written to a location outside `intelligence/companies/` (e.g. an ephemeral drafting workspace, never committed as a tracked evidence-of-record file — the redacted package is a drafting aid, not a retained artifact; the retained artifact is the sealed classification record itself plus its evidence citations back to the original, unredacted Company Intelligence record for later Milestone-7 audit).

### 7.3 Why a marker list plus mandatory verification, not a marker list alone

This artifact's own §7.1 finding is direct proof that a plausible-sounding claim about a file's content ("evidence-only by construction") can be wrong when checked against the actual 27-ticker corpus rather than assumed from the frozen spec's intent. The same risk applies to the marker list in step 4 itself — a future record could phrase a policy conclusion in a way this list doesn't catch. Step 5's mandatory re-scan-and-fail-hard rule is the structural answer to that risk: the redaction procedure does not merely apply a filter and trust it, it verifies the filter's own output before that output is allowed to reach a drafting session, and treats any surviving match as a stop condition rather than a tolerable miss.

### 7.4 Determinism and auditability

Steps 1-3 operate on a fixed, named key list; steps 4-5 operate on a fixed marker list plus a mandatory verification pass rather than free-form judgment. The same script or process run twice against the same source `main` commit produces byte-identical redacted output for both the `.yaml` and `.md` portions of every ticker's package — auditable by any future independent reviewer by re-running the same deterministic procedure and diffing. **No redacted package is created and no such script is implemented by this filing** — §7.2 is a complete specification for a future, separately authorized implementation PR to build against, matching this repository's established design-then-implement pattern (`TIER-0002` designed the classification schema itself without creating a single `intelligence/classification/` file).
