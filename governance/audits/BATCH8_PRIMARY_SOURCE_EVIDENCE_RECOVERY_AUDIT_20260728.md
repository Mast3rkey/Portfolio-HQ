...
**Known, disclosed, non-blocking artifact characteristic — reverse-index reciprocity.** This session independently recomputed, directly from the frozen YAML, the full set of unique claim-level `(claim_id, source_id)` pairs (derived from every claim's own `source_ids`) and the full set of unique reverse-index `(claim_id, source_id)` pairs (derived from every source's own `claims_supported`): **108 unique claim-level pairs, 88 unique reverse-index pairs, 85 pairs in the intersection.** The source-manifest reverse index is nonreciprocal with claim-level provenance: 23 claim-level `(claim_id, source_id)` pairs are absent from `claims_supported`, while 3 reverse-index pairs are absent from the corresponding claim-level `source_ids`, for 26 asymmetric pairs total. Claim-level `source_ids` are controlling provenance. The three reverse-index-only pairs — cases where a source's `claims_supported` names a claim that does not itself cite that source back — are `BATCH-C7`/`ORCL-S1`, `CRWD-C4`/`CRWD-S4`, and `IBM-C3`/`IBM-S2`. This audit does not repair the bundle's `claims_supported` index; every Company Intelligence record produced from this bundle cites its evidence from each claim's own `source_ids` and `exact_locators`, not from the source-side reverse index.
