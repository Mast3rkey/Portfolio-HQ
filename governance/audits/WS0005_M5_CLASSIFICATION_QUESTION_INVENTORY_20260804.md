...
**Would separation change an outcome?** Yes, materially — this is the one axis where CLAUDE.md's own Decisions Log already documents a live doctrine gap: "T1 and T2 have no trim rule at all... That's a bigger doctrine question" (the 2026-07-19 cluster-cap-declined entry), later partly addressed by a T1/T2 ceiling trim whose cost was explicitly recorded as "untested, not tested and cheap" (Open Items). 19 of 27 canonical names belong to zero cluster cap and zero relationship record — risk concentration for the majority of the roster is currently unmeasured by any mechanism.
...
| Risk concentration/overlap | Partially (3 clusters cover 8/27; 19/27 uncovered) | **Retain for further consideration** — the one axis with a documented live doctrine gap (T1/T2 trim history) |
