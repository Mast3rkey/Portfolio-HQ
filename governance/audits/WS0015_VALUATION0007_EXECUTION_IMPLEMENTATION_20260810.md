...
## 13. Bounded correction (same day, this PR)

An independent exact-head review, posted as a `COMMENT` on PR #291 (GitHub rejected `APPROVE`/`REQUEST_CHANGES` — the reviewing session and the PR-authoring session share the same GitHub account, `Mast3rkey`, the same disclosed platform restriction this repository's history has repeatedly recorded), returned **CHANGES REQUIRED — 0 BLOCKING / 3 MAJOR / 3 MINOR / 2 NOTE**, anchored to exact head `eaac705a32afb582728f32490d6ad0e0a02aea74`. All three MAJOR findings and MINOR-2/MINOR-3 were independently reproduced against this session's own live data before any fix, confirmed genuine, and resolved. MINOR-1 (a PR-body header arithmetic slip, "33" vs the actual 32 files) is a PR-description text fix, applied separately to the PR body, not to any repository file. Both NOTE items were non-actionable by the review's own characterization.

**MAJOR-1 — SOTP (`family_1`) low/high sensitivity multipliers were used to compute the reported range but never logged to the `assumptions_ledger`, resolved.** Independently reproduced: `AMZN.yaml`'s ledger listed only `na_ebit_multiple_base`/`intl_ebit_multiple_base`/`aws_ebit_multiple_base` (all `15`), while the reported `low`/`high` implied multipliers of `12`/`19` (matching the code's own then-flat `EBIT_BAND = (12, 15, 19)` constant) appeared nowhere in the ledger — a reader could not reproduce or verify the range from the record's own disclosed inputs. Fixed at the source: `sotp_family()` (`/tmp/valgen/build_family.py`, this session's own scratch generator, not a repository file) now logs all three tiers (`_low`/`_base`/`_high`) per segment, matching the `family_5` P/E-style records' own pre-existing pattern. All 7 `completed` SOTP records (`AMZN`, `AVGO`, `GOOGL`, `MSFT`, `RTX`, `SPGI`, `TSLA`) regenerated; every segment now carries three fully provenance-labeled, verifiable ledger entries.

**MAJOR-2 — SOTP intra-company segment multiples were uniform, mathematically collapsing several "sum-of-the-parts" results into a single blended multiple, resolved for the 3 records with real per-segment evidence; disclosed for the 4 without.** Independently confirmed: every profit-bearing segment across all 7 SOTP records previously got the identical flat `EBIT_BAND`, regardless of the segment's own real, disclosed margin — AMZN's AWS (35.4% margin) and North America (6.9% margin) both got `15x`. Root-caused and fixed with genuine, evidence-grounded differentiation for the 3 records where Stage-3 evidence discloses real per-segment profit (`AMZN`, `MSFT`, `RTX`): each profit-bearing segment's own margin (`profit/revenue`) now selects its own EV/EBIT multiple tier from a new three-tier band (`>=35%` high `14/17/21`; `20–35%` mid `11/14/17`; `<20%` low `8/10/13`) — AMZN's AWS now lands in the high tier while North America/International land in the low tier, a real differentiation grounded in real numbers, not an artificial uniform assumption. RTX's Collins Aerospace (4.6% margin) and Pratt & Whitney (2.4% margin) land in the same low tier on this corrected, margin-driven basis — confirmed a genuine finding (both segments really do have similarly thin margins), not a leftover uniform default. For the 4 records with **no** segment-level profit disclosed anywhere in Stage-3 evidence (`AVGO`, `GOOGL`, `SPGI`, `TSLA`), no stronger evidence exists to differentiate beyond the company's own overall margin — genuinely differentiating further would mean inventing figures with no evidentiary basis, which this correction declines to do. Instead, each of the 4 records' `uncertainty_summary` now states explicitly and quantitatively that the uniform tier makes `sum(segment_revenue × multiple) = multiple × total_revenue` for that specific record — the SOTP framing is disclosed as not capturing genuine part-by-part differentiation in these 4 cases, rather than leaving that mathematical consequence implicit.

**MAJOR-3 — undisclosed peer exclusions across 14 of 27 `family_5` records, and the retained audit's own contrary claim in §7, resolved.** Independently reproduced by comparing every record's `applied_peers` against its pinned Stage-3 evidence record's own `included` peer candidates: **exactly 14 tickers** (`ASML`, `COST`, `ETN`, `ICE`, `ISRG`, `LLY`, `META`, `NVDA`, `PANW`, `PWR`, `SNPS`, `TMO`, `V`, `WM`) had silently excluded one of three Stage-3-included candidates with zero disclosed rationale — in 13 of the 14 cases the specific third-listed candidate, a mechanical-truncation signature, not an individualized comparability judgment. This directly contradicted this document's own original §7 claim that peer-set selection was "not exercised" and that "every included peer named in Stage-3 evidence was cited as comparability context" — both clauses were false as originally written. Resolved by including **all** Stage-3-included peer candidates in every affected record's `applied_peers` (no candidate is excluded without a disclosed reason anywhere in the corrected corpus) — since there was no genuine comparability-based reason to exclude any of them, the correct fix is inclusion, not after-the-fact rationale for an arbitrary drop. This also mechanically resolved `ICE.yaml`/`V.yaml`'s internal inconsistency (body text claiming "three included peers" against a 2-entry `applied_peers` list) — both now correctly show all 3. §7's claim is corrected below (§14).

**MINOR-2 — leftover `"(for RKLB)"` template artifact in `GNRC.yaml`/`KLAC.yaml`, resolved.** Both records' `governing_sensitivity` text was copy-pasted from `RKLB.yaml`'s own isolation-compliance sentence, including a parenthetical referencing RKLB by name inside GNRC's/KLAC's own records — nonsensical, since neither assigns a probability weight. Split into two distinct isolation-note variants: a scenario-value-only note (`GNRC`, `KLAC`) and a scenario-value-and-probability-weight note (`RKLB` only) — no cross-reference to another ticker survives in either.

**MINOR-3 — `latest_fy_pe`/`latest_fy_pfcf` ledger entries stored a dollar base metric under a ratio-shaped name, resolved.** Every `family_5` record's dollar base-metric entry (net income or FCF) is now named `latest_fy_net_income` or `latest_fy_fcf` as appropriate — the multiple entries themselves (`{metric}_multiple_low/base/high`) are unchanged, since those genuinely are ratios.

**No change to `result_status` or `family_status` for any of the 27 tickers** — result-status distribution independently reconfirmed unchanged at **18 `completed` / 9 `partial` / 0 `unable_to_determine`**; these were disclosure-quality and methodology-honesty corrections, not status changes. Full validation re-run clean at the corrected head: `valuation_result_validator.py` → `OK (28 result(s))`; `test_valuation_result_validator.py` → **176 passed**; all 11 other repository validators clean; manifest/hash reconciliation 27/27, zero mismatches, deterministic; `validate_authorized_cohort()` clean against the exact 27-name population; `git diff --check` clean.

## 14. Correction to §7's peer-selection claim

§7 above (unedited, preserved as the original narrative) states peer-set selection was "not exercised" and that every included peer was cited. Following the §13 correction, that statement no longer describes the corrected corpus's own history accurately as a *retrospective* claim about the original submission (which did, in fact, silently truncate 14 records) — but the corrected state itself now genuinely has no exclusion anywhere: every Stage-3-included peer candidate appears in every affected record's `applied_peers`, so there is no longer a selective peer-exclusion judgment requiring isolation-rule disclosure for any of the 27 tickers. RKLB's own scenario-probability isolation disclosure (§7, unedited) remains accurate and unaffected.
