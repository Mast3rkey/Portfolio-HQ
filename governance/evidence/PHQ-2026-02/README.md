...
- SHA-256 (verified against the principal-supplied expected value before any
  file was read): `fb8eb811df29eb560bfaed16e8d0e89c6cbcf44bc42e1f1ae2ccfca4cddd889e`.
- Every file's SHA-256 and byte count were independently re-derived and
  matched the archive's own `MANIFEST.json` before use.
...
The v1.35 package captures 24 equity/fund positions + SPCX + 7 crypto entries
(31 total) — far fewer than the 65 equity/fund tickers `holdings.yaml`
tracked before this reconciliation. The package's own README states the later
...
every non-dust captured quantity (24 equity/fund + SPCX + BTC/ETH/SOL, 27
rows) maps exactly to the corresponding `holdings.yaml` entry
